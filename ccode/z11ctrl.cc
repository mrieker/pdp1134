...
// continuously display what would be on the PDP-11 front panel
// reads panel lights and switches from z11panel via udp

// requires './z11panel -status server' running in background on zturn
...
static int showstatus (int argc, char **argv)
{
    ...
    while (true) {
        ...
        // display it
        printf (TOP EOL);
        printf ("  PDP-11       %s   %s %s %s   %s %s %s   %s %s %s   %s %s %s  " ESC_BOLDV "%o.%04o" ESC_NORMV "  MA   IR [ %s %s %s  " ESC_ON "%s" ESC_NORMV " ]     %10u fps" EOL,
