...
#define MAXCTR 32       // PORTST3 max length of counter data block
...
static uint32_t rdrca;                  // receive descriptor current address
static uint32_t tdrba, tdrca;           // transmit descriptor ring base address, current address
static Z11Page *z11p;
...
static void xeiothread ()
{
    ...
    while (true) {
        ...
        if (! (pcsr0 & HIJK)) continue;         // make sure hijack bit is set

        if (pcsr0 & RSET) {                     // check for reset
            doreset ();
            writepcsr0 (RSET | HIJK);           // set port command done bit
        }
            ...
            writepcsr0 (DNI | HIJK);            // set port command done bit
        }
...
static void getcommand (uint32_t pcbaddr)
{
    ...
    if (z11p->dmaread (pcbaddr, &pcbdata[0]) != 0) goto dmaerror;
    switch (pcbdata[0] & 0xFF) {

        // get default physical address
        case 002: {
            if (! z11p->dmawrite (pcbaddr + 2, defethaddr[0])) goto dmaerror;
            if (! z11p->dmawrite (pcbaddr + 4, defethaddr[1])) goto dmaerror;
            if (! z11p->dmawrite (pcbaddr + 6, defethaddr[2])) goto dmaerror;
            break;
        }

        // get current physical address
        case 004: {
            if (! z11p->dmawrite (pcbaddr + 2, curethaddr[0])) goto dmaerror;
            if (! z11p->dmawrite (pcbaddr + 4, curethaddr[1])) goto dmaerror;
            if (! z11p->dmawrite (pcbaddr + 6, curethaddr[2])) goto dmaerror;
            break;
        }

        // set current physical address
        case 005: {
            if (z11p->dmaread (pcbaddr + 2, &curethaddr[0]) != 0) goto dmaerror;
            curethaddr[0] &= 0xFFFEU;
            if (z11p->dmaread (pcbaddr + 4, &curethaddr[1]) != 0) goto dmaerror;
            if (z11p->dmaread (pcbaddr + 6, &curethaddr[2]) != 0) goto dmaerror;
            break;
        }
        ...
        case 006: {
            ...
            if (z11p->dmaread (pcbaddr + 2, &udbblo) != 0) goto dmaerror;
            if (z11p->dmaread (pcbaddr + 4, &udbbhi) != 0) goto dmaerror;
            uint32_t udbb = (((uint32_t) udbbhi << 16) | udbblo) & 0777776;
            uint16_t mltlen = udbbhi >> 8;
            for (uint16_t i = 0; (i < 10) && (i < mltlen); i ++) {
                if (! z11p->dmawrite (udbb + i * 6 + 0, mltaddrtable[i*3+0])) goto dmaerror;
                if (! z11p->dmawrite (udbb + i * 6 + 2, mltaddrtable[i*3+1])) goto dmaerror;
                if (! z11p->dmawrite (udbb + i * 6 + 4, mltaddrtable[i*3+2])) goto dmaerror;
            }
            break;
        }
        ...
        case 007: {
            ...
            uint16_t mltlen = udbbhi >> 8;
            for (uint16_t i = 0; (i < 10) && (i < mltlen); i ++) {
                if (z11p->dmaread (udbb + i * 6 + 0, &mltaddrtable[i*3+0])) goto dmaerror;
                mltaddrtable[i*3+0] |= 1;
                if (z11p->dmaread (udbb + i * 6 + 2, &mltaddrtable[i*3+1])) goto dmaerror;
                if (z11p->dmaread (udbb + i * 6 + 4, &mltaddrtable[i*3+2])) goto dmaerror;
            }
            break;
        }
        ...
        case 010: {
            ...
            for (uint32_t i = 0; i < 6; i ++) {
                if (! z11p->dmawrite ((udbb + i * 2) & 0777776, rfbuf[i])) goto dmaerror;
            }
            break;
        }
        ...
        case 011: {
            ...
            uint32_t udbb = (((uint32_t) udbbhi << 16) | udbblo) & 0777776;
            for (uint32_t i = 0; i < 6; i ++) {
                if (z11p->dmaread ((udbb + i * 2) & 0777776, &rfbuf[i]) != 0) goto dmaerror;
            }
        ...
        case 013: {
            ...
            if (z11p->dmaread (pcbaddr + 2, &udbblo) != 0) goto dmaerror;
            if (z11p->dmaread (pcbaddr + 4, &udbbhi) != 0) goto dmaerror;
            if (z11p->dmaread (pcbaddr + 6, &ctrlen) != 0) goto dmaerror;
            uint32_t udbb = (((uint32_t) udbbhi << 16) | udbblo) & 0777776;
            ...
            for (uint16_t i = 0; counters.ubusdblen; i ++) {
                if (! z11p->dmawrite (udbb + i * 2, ((uint16_t *)&counters)[i])) goto dmaerror;
            }
            if (pcbdata[0] & 1) {
                memset (&counters, 0, sizeof counters);
                counterszeroed = time (NULL);
        ...
        case 014: {
            if (! z11p->dmawrite (pcbaddr + 2, modebits)) goto dmaerror;
            break;
        }
        case 015: {
            if (z11p->dmaread (pcbaddr + 2, &modebits) != 0) goto dmaerror;
            modebits &= 0xFA0F;
            break;
        }
        ...
        case 017: {
            ...
            if (! z11p->dmawrite (pcbaddr + 2, portst1 | RREV)) goto dmaerror;
            if (! z11p->dmawrite (pcbaddr + 4, portst2)) goto dmaerror;
            if (! z11p->dmawrite (pcbaddr + 6, MAXCTR)) goto dmaerror;
        ...
        if ((pcsr1 & RUN) && matchesincoming (rcvbuf)) {
            if (ringfmt.rrlen == 0) {
                ...
                do {
                    ...
                    uint16_t rdrb[3];
                    for (int i = 0; i < 3; i ++) {
                        if (z11p->dmaread ((rdrca + i * 2) & 0777776, &rdrb[i]) != 0) goto dmaerror;
                    }
                    if (! (rdrb[2] & 0x8000)) {
                        status |= RCBI;                     // set RCBI - receive buffer unabailable interrupt
                        break;
                    }
                    ...
                    if (index == 0) byte5 |= 0x02;          // STP - start of packet
                    if (modebits & MODE_DRDC) word6 |= 0x2000U;  // NCHN - not in chaining mode
                ...
                badbuf:;
                    ...
                    // if no further entry, assume it is owned by the PDP
                             ...
                             (z11p->dmaread ((rdrna + 4) & 0777776, &nextowner) != 0) ||
                             ! (nextowner & 0x8000U)) {
                        word6 |= 0x8000U;                   // BUFL - buffer length error
                        numbytestogo = amountfits;          // chop!
                    }

                    // write status out to memory, turning ownership over to PDP
                    if (! z11p->dmawrite (rdrca + 6, word6)) goto dmaerror;
                    if (! z11p->dmawbyte (rdrca + 5, byte5)) goto dmaerror;
...
static void *transmithread (void *dummy)
{
    ...
    while (true) {
        ...
        if (! (pcsr1 & RUN) || (ringfmt.trlen == 0) || ! didsomething) {
            ...
            // p108
            for (int i = 0; i < 4; i ++) {
                if (z11p->dmaread ((tdrca + i * 2) & 0777776, &tdrb[i]) != 0) goto dmaerror;
            }
            ...
            if (index == 0xFFFFFFFFU) {             // looking for start of buffer (STP)
                if (! z11p->dmawrite (tdrpa + 6, tdrb[3] | 0x8000U)) goto dmaerror;
                continue;                           // not found, set BUFL then check next descriptor
            }

            if (index + tdrb[0] > sizeof xmtbuf) {  // see if message too int
                if (! z11p->dmawrite (tdrpa + 6, tdrb[3] | 0x8000U)) goto dmaerror;
                index = 0xFFFFFFFFU;
                continue;                           // if so, set BUFL then check next descriptor
            }
            ...
            if (! z11p->dmawrite (tdrpa + 4, tdrb[2] & 0x7FFFU)) goto dmaerror;
        }
        goto done;
    dmaerror:;
